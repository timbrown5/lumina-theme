...
interface CustomizePanelProps {
  ...
  themeLogic: any; // Pass the whole theme logic for access to all methods
}
...
const CustomizePanel: React.FC<CustomizePanelProps> = ({
  ...
  // Get current parameters from themeLogic
  const params = themeLogic.getCurrentParams();

  const getSliderProps = (configKey: string) => {
        ...
        <>
          ...
          <div className="mb-5">
            ...
            <ColorPaletteEditor
              ...
              onResetAdjustment={themeLogic.resetColorAdjustment}
...
const PreviewPanel: React.FC<PreviewPanelProps> = ({ colors, activeTab, setActiveTab }) => {
  ...
  // Remove the 'colors' tab since we moved the palette to the customization panel
  const availableTabs = TABS.filter(tab => tab.id !== 'colors');
  ...
  >
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
      ...
      <Button
        ...
        variant="gradientRed"
        colors={pageColors}
        className="px-4 py-3import React from 'react';
import {
  getBaseTheme,
  getAllThemeKeys,
  getAllFlavorKeys,
...
interface CustomizePanelProps {
  ...
  pageColors: Base24Colors;
  onExpandChange: (expanded: boolean) => void;
  themeLogic: any; // We'll pass the whole theme logic for the color palette
}
...
const App: React.FC = () => {
  ...
  // Set default tab to ui-preview since we removed colors tab
  React.useEffect(() => {
